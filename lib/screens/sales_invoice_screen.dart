...
class SalesInvoiceScreen extends StatefulWidget {
  ...
  const SalesInvoiceScreen({super.key, this.onBack});
...
class _SalesInvoiceScreenState extends State<SalesInvoiceScreen> {
  ...
  Widget _buildBody() {
    ...
    if (_filteredInvoices.isEmpty) {
      return EmptyState(
        ...
        subtitle: _searchController.text.isNotEmpty ? 'Try a different search term or check filters' : 'Sales invoice data will appear here once synced from Tally',
        onRetry: _loadData,
      );
    }
  ...
  Future<void> _pickCustomDateRange() async {
    ...
    if (picked != null) {
      ...
      UserPreferencesService.saveSalesDateFilter('custom');
      _applyFiltersAndSort();
    }
  }
  ...
  Widget _buildDateFilterPills() {
    final isCustom = _dateRangeOption == DateRangeOption.custom;
    final customLabel = (isCustom && _selectedDateRange != null)
  ...
  Widget _buildCalendarPill(String? customLabel) {
    final isSelected = _dateRangeOption == DateRangeOption.custom;
    final activeColor = AppTheme.salesColor;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          padding: EdgeInsets.symmetric(
            horizontal: customLabel != null ? 10 : 8,
          ),
          decoration: BoxDecoration(
            color: isSelected ? activeColor : Colors.white,
            borderRadius: BorderRadius.circular(8),
  ...
  void _showInvoiceDetail(SalesInvoice invoice) {
    final dateFormat = DateFormat('dd MMM yyyy');
                      ...
                      children: [
                        Row(
                          children: [
                            Container(
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(color: AppTheme.salesColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(12)),
                              child: const Icon(Icons.receipt_long_rounded, color: AppTheme.salesColor, size: 24),
                            ),
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                try {
                                  ...
                                  await PdfService.shareSalesInvoice(invoice, items);
                                } catch (e) {
                                  if (context.mounted) {
                        ...
                        _detailSection('Customer Details', [
                          _detailItem('Customer', invoice.customerName),
                          if (invoice.shippingAddress != null && invoice.shippingAddress!.isNotEmpty)
                            _detailItem('Ship To', invoice.shippingAddress!),
                          if (invoice.customerCategoryName != null && invoice.customerCategoryName!.isNotEmpty)
                            _detailItem('Category', invoice.customerCategoryName!),
                        ...
                        if (invoice.id != null) ...[
                          ...
                          FutureBuilder<List<InvoiceItem>>(
                            future: _service.getInvoiceItems(invoice.id!),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(
                  children: [
                    ...
                    Text(formatCurrency(item.totalAmount), style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppTheme.salesColor)),
                  ],
                ),
                const SizedBox(height: 6),
            ...
            Row(
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.salesColor.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
                  child: const Icon(Icons.receipt_long_rounded, color: AppTheme.salesColor, size: 20),
                ),
                const SizedBox(width: 12),
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(invoice.customerName, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Color(0xFF1A1F36))),
                ...
                Column(
                  ...
                  children: [
                    Text(formatCurrency(invoice.totalAmount), style: const TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.salesColor)),
                    if (invoice.invoiceDate != null)
            ...
            Row(
              children: [
                ...
                if (invoice.type != null && invoice.type!.isNotEmpty)
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                    decoration: BoxDecoration(color: AppTheme.salesColor.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(6)),
                    child: Text(invoice.type!, style: const TextStyle(fontSize: 11, color: AppTheme.salesColor, fontWeight: FontWeight.w600)),
                  ),
                const Spacer(),
